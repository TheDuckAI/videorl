...
def load_data(input_csv_path, config):
    """
    ...
    if "quality_score" not in data[0]:
        ...
        for row in data:
            video_id = row["id"]
            quality_score = compute_quality_score(video_id, config)  # This function is from utils.py
...
def train_model(input_csv_path, model_path, config_path):
    ...
    X, y = load_data(input_csv_path, config)
